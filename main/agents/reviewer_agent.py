...
3. Выдаем вердикт approve или request changes(в формате Вердикт: результат  ).
4. Комментируем PR с объяснением. ОБЯЗАТЕЛЬНО КОММЕНТАРИИ. Но краткие выжимки
5. Отвечаем на русском языке.
6. Если в проекте не обнаружено было CI тестов, напиши об этом и попытайся проанализировать без CI тестов
